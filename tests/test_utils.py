...
@pytest.mark.parametrize(
    "value_in, value_out",
    [(-1, 0), (1, 1), (10000, 10000), ("-1", 0), ("1", 1), ("10000", 10000)],
)
...
class TestCheckClient:
    @pytest.mark.parametrize(
        "value_in, bool_out", [("111.111.111.111", True), ("111.111.111.1111", False)]
    )
    ...
    @pytest.mark.parametrize(
        "value_in, bool_out",
        [("1.1.1.1", True), ("1.1.1", False), ("1.1.1.1.1", False)],
    )
    ...
    @pytest.mark.parametrize(
        "value_in, bool_out",
        [("1.2.32.10", True), ("1.2.34.xxx", False), ("x.x.x.x", False)],
    )
    ...
    @pytest.mark.parametrize(
        "value_in, bool_out",
        [("0.0.0.0", True), ("255.255.255.255", True), ("1.1.1.300", False)],
    )
